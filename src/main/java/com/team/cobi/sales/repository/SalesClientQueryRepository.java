...
@Slf4j
@Repository
public class SalesClientQueryRepository extends QuerydslRepositorySupport {

    private final JPAQueryFactory jpaQueryFactory;

    // QuerydslRepositorySupport 쓰기 위해서 @RequiredArgsConstructor 사용 안하고 직접 생성
    public SalesClientQueryRepository(JPAQueryFactory jpaQueryFactory) {
    ...
    public SalesClientCodeResponse clientItem(String id) {
        return jpaQueryFactory
                .select(Projections.bean(SalesClientCodeResponse.class,
                        ...
                        client.telephone,
                        orderForm.status,
                        //orderForm.createdDate.as("orderFormCreatedDate"),
                        product.productName,
                        product.category,
                        product.unitPrice,
                ...
                .leftJoin(orderForm)
                .on(salesEstimate.id.eq(orderForm.estimateId))
                .leftJoin(product)
    ...
    private BooleanExpression likeClientName(String clientName) {
        log.info("sales likeClientName 너는 탔지? " + clientName);
